#!/bin/bash
#SBATCH --job-name=ol-v8a-dit-t320
#SBATCH --partition=inference-large
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=6:00:00
#SBATCH --output=logs/v8a_dit_pixel_tile320_overlap160_%j.out
#SBATCH --error=logs/v8a_dit_pixel_tile320_overlap160_%j.err
...
OUT_DIR=${OUT_DIR:-/scratch/emboulaalam/OceanLens_git/results/v8a_dit_pixel_inference/heun50_sigma10_ens1_tile320_overlap160}
...
for variable in thetao so zos uo vo; do
  python scripts/plot_one_variable_residuals.py \
    ...
    --variable "${variable}" \
    --title "v8a DiT | ${variable} | Heun 50 | sigma=1.0 | ens=1 | tile=320 overlap=160 | 2004-01-01"
done
