...
from oceanlens.models.fm_dit import FlowMatchingDiT
from oceanlens.models.fm_swin import FlowMatchingSwinUNet
...
def build_fm(cfg, ckpt: Path, device: torch.device):
    ...
    elif fm_arch == "swin_unet":
        model_cfg = getattr(cfg, "model", {})
        cond_cfg = getattr(cfg, "condition", {})
        fm = FlowMatchingSwinUNet(
            x_channels=int(getattr(cfg.fm, "x_channels", cfg.fm.out_channels)),
            condition_channels=int(getattr(cond_cfg, "condition_channels", cfg.fm_in_channels - cfg.fm.out_channels)),
            dynamic_channels=int(getattr(cond_cfg, "dynamic_channels", 5)),
            out_channels=cfg.fm.out_channels,
            patch_size=int(getattr(model_cfg, "patch_size", 4)),
...
def distance_to_coast(mask: np.ndarray) -> np.ndarray:
    """Return a normalized distance-to-coast channel from a 1xHxW ocean mask."""
    ocean = mask[0] > 0.5
    dist = distance_transform_edt(ocean).astype(np.float32)
    scale = np.percentile(dist[ocean], 99.0) if ocean.any() else 1.0
    scale = max(float(scale), 1.0)
    return (np.clip(dist / scale, 0.0, 1.0)[None, ...] * mask).astype(np.float32)


def region_id(mask: np.ndarray, lat, lon) -> np.ndarray:
    """Build the same seven one-hot region channels used during training."""
    _, height, width = mask.shape
    if lat is None or len(lat) != height:
        lat = np.linspace(-90.0, 90.0, height, dtype=np.float32)
    if lon is None or len(lon) != width:
        lon = np.linspace(-180.0, 180.0, width, endpoint=False, dtype=np.float32)
    lon = ((np.asarray(lon) + 180.0) % 360.0) - 180.0
    lat_grid, lon_grid = np.meshgrid(np.asarray(lat), lon, indexing="ij")
    ocean = mask[0] > 0.5
    ...
    coastal = np.minimum(dist_ocean, dist_land) <= 3.0
...
def geo_coordinates(mask: np.ndarray, lat, lon) -> Dict[str, np.ndarray]:
    """Build normalized latitude and periodic longitude channels."""
    _, height, width = mask.shape
    if lat is None or len(lat) != height:
        lat = np.linspace(-90.0, 90.0, height, dtype=np.float32)
    if lon is None or len(lon) != width:
        lon = np.linspace(-180.0, 180.0, width, endpoint=False, dtype=np.float32)
    lon = ((np.asarray(lon) + 180.0) % 360.0) - 180.0
    lat_norm = np.clip(np.asarray(lat, dtype=np.float32) / 90.0, -1.0, 1.0)
    ...
    return {
        "lat": (lat_grid[None, ...] * mask).astype(np.float32),
        "lon_sin": (lon_sin[None, ...] * mask).astype(np.float32),
        "lon_cos": (lon_cos[None, ...] * mask).astype(np.float32),
    }
...
@torch.no_grad()
def infer_tile(cno, fm, cfg, lr: np.ndarray, mask: np.ndarray, extra: Dict[str, np.ndarray], args, device: torch.device):
    ...
    extra_t = {key: torch.from_numpy(value).unsqueeze(0).to(device) for key, value in extra.items()}
    ...
    condition = build_fm_condition(mu, lr_t, mask_t, cfg, extra=extra_t)
    samples = []
    fm_outputs = []
    dt = 1.0 / args.n_steps
    ...
    def velocity(state, time):
        ...
        v_cond = fm(state, time, condition)
        v_uncond = fm(state, time, uncond_condition)
        return v_uncond + args.cfg_scale * (v_cond - v_uncond)
...
def infer_tiled(cno, fm, cfg, lr: np.ndarray, mask: np.ndarray, extras: Dict[str, np.ndarray], args, device: torch.device):
    ...
    for top in y_starts:
        ...
        for left in x_starts:
            ...
            pred, fm_residual, mu, cno_residual = infer_tile(
                ...
                {key: value[:, top:bottom, left:right] for key, value in extras.items()},
                args,
                device,
            )
...
def main() -> None:
    ...
    # 4. Load either paired yearly files or one saved NPZ test case.
    ds_hr = ds_lr = None
    if args.input_npz is not None:
        ...
        print(f"Running minimal inference from NPZ for {date}", flush=True)
    else:
    ...
    extras = {
        "siconc": np.zeros((1, *hr_norm.shape[-2:]), dtype=np.float32),
        "dist_coast": distance_to_coast(mask),
        "region_id": region_id(mask, lat, lon),
    }
    extras.update(geo_coordinates(mask, lat, lon))

    # 7. Run tiled inference and record GPU memory/time.
    if device.type == "cuda":
        torch.cuda.reset_peak_memory_stats(device)
    start = time.perf_counter()
    pred_norm, fm_norm, mu_norm, cno_residual_norm = infer_tiled(
        cno,
        fm,
        cfg,
        lr_model_norm,
